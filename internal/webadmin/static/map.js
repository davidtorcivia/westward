...
(() => {
  ...
  var home = [parseFloat(el.dataset.lat || "40.6782"), parseFloat(el.dataset.lon || "-73.9442")];
  ...
  fetch("/admin/map/data")
    ...
    .then((d) => {
      ...
      L.circleMarker([d.home.lat, d.home.lon], {
        radius: 7, color: "#f5b871", weight: 2,
        fillOpacity: 0.9, fillColor: "#e8875f",
      }).addTo(map).bindTooltip("watch location");

      (d.cameras || []).forEach((c) => {
        if (c.Lat == null || c.Lon == null) return;
        L.circleMarker([c.Lat, c.Lon], {
          radius: 8, color: "#ffd9a8", weight: 2,
          fillOpacity: 0.95, fillColor: "#ff9d5c",
        })
          .addTo(map)
      ...
      (d.dot || []).forEach((c) => {
        var m = L.circleMarker([c.Lat, c.Lon], {
          radius: 3, color: "rgba(210,180,160,0.45)", weight: 1,
          fillOpacity: c.Online ? 0.55 : 0.15, fillColor: "#c9a08a",
        }).addTo(map);
